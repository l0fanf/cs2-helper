...
LANG = {
    "en": {
        ...
        "action_diagnostics": "Run Diagnostics",
        "health_title": "System Health",
        "health_parser": "Parser Engine",
        "health_pygame": "Replay Renderer",
        "health_openai": "Neural API",
        "health_ok": "Operational",
        "health_missing": "Missing",
        "health_disabled": "Disabled",
        "settings_title": "Experience Settings",
        "settings_motion": "Enable Motion Effects",
        "settings_aurora": "Enable Aurora Background",
        "settings_contrast": "High Contrast Mode",
        "settings_save": "Apply Settings",
        "faceit_status": "Sync Status",
        "faceit_ready": "Ready",
        "faceit_syncing": "Syncing...",
        "faceit_failed": "Sync failed",
        "faceit_retry": "Retry"
    },
    "zh": {
        "start": "开启心流分析",
        ...
        "health_disabled": "已关闭",
        "settings_title": "体验设置",
        "settings_motion": "开启动态效果",
        "settings_aurora": "开启极光背景",
        "settings_contrast": "高对比度模式",
        "settings_save": "应用设置",
        "faceit_status": "同步状态",
        "faceit_ready": "就绪",
        "faceit_syncing": "同步中...",
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.enable_animations = CONFIG.get("enable_animations", True)
        self.enable_aurora = CONFIG.get("enable_aurora", True)
        ...
        self.bind("<Control-o>", lambda event: self.select_demo_file())
        self.bind("<Control-e>", lambda event: self.export_report())
        self.animate_loop()
        self.update_clock()
    ...
    def setup_background(self):
        if HAS_PIL and self.enable_aurora:
    ...
    def apply_visual_preferences(self):
        if hasattr(self, "content"):
            self.content.configure(fg_color="#0b0b0b" if self.high_contrast else C_SURFACE)
        if hasattr(self, "nav"):
            self.nav.configure(fg_color="#0b0b0b" if self.high_contrast else C_SURFACE)
        if self.enable_aurora:
            if not hasattr(self, "bg_canvas"):
                self.setup_background()
        elif hasattr(self, "bg_canvas"):
    ...
    def show_page(self, key):
        ...
        header_map = {
            "tab_dash": LANG[self.lang]["tab_dash"],
            "tab_round": LANG[self.lang]["tab_round"],
            "tab_ai": LANG[self.lang]["tab_ai"],
            "tab_faceit": LANG[self.lang]["tab_faceit"],
            "tab_util": LANG[self.lang]["tab_util"],
            "tab_replay": LANG[self.lang]["tab_replay"],
        }
        self.lbl_header.configure(text=header_map.get(key, ""))
    ...
    def build_dashboard(self, parent):
        ...
        self.lbl_health_title = ctk.CTkLabel(activity_card, text=LANG[self.lang]["health_title"], font=("Helvetica", 13, "bold"), text_color=C_HIGHLIGHT)
        self.lbl_health_title.pack(anchor="w", padx=24, pady=(0, 6))
        self.health_items = {}
        for key, label in [
            ("parser", LANG[self.lang]["health_parser"]),
            ("pygame", LANG[self.lang]["health_pygame"]),
            ("openai", LANG[self.lang]["health_openai"]),
        ]:
            row = ctk.CTkFrame(activity_card, fg_color="transparent")
            row.pack(fill="x", padx=24, pady=2)
            name_lbl = ctk.CTkLabel(row, text=label, font=("Helvetica", 12), text_color=C_DIM)
            name_lbl.pack(side="left")
            status_lbl = ctk.CTkLabel(row, text="—", font=("Helvetica", 12, "bold"), text_color=C_DIM)
            status_lbl.pack(side="right")
            self.health_items[key] = {"label": name_lbl, "status": status_lbl}

        action_row_2 = ctk.CTkFrame(actions, fg_color="transparent")
        action_row_2.pack(fill="x", pady=(10, 0))
        self.btn_settings = self.create_action_btn(action_row_2, LANG[self.lang]["action_settings"], self.open_settings_modal, C_NEON_BLUE)
        self.btn_diagnostics = self.create_action_btn(action_row_2, LANG[self.lang]["action_diagnostics"], self.run_diagnostics, C_STATUS_GREEN)
    ...
    def run_diagnostics(self):
        status_map = {
            "parser": (HAS_PARSER, LANG[self.lang]["health_ok"], LANG[self.lang]["health_missing"]),
            "pygame": (HAS_PYGAME, LANG[self.lang]["health_ok"], LANG[self.lang]["health_missing"]),
            "openai": (HAS_OPENAI, LANG[self.lang]["health_ok"], LANG[self.lang]["health_disabled"]),
        }
        for key, (available, ok_text, missing_text) in status_map.items():
            if key not in self.health_items:
                continue
            status = ok_text if available else missing_text
            color = C_STATUS_GREEN if available else C_STATUS_RED
            self.health_items[key]["status"].configure(text=status, text_color=color)
        self.notify("Diagnostics updated", C_ACCENT_2)
        self.log_activity("Diagnostics refreshed")

    def open_settings_modal(self):
        modal = ctk.CTkToplevel(self)
        modal.title(LANG[self.lang]["settings_title"])
        modal.geometry("420x360")
        modal.configure(fg_color=C_BG_BASE)
        modal.transient(self)
        modal.grab_set()

        ctk.CTkLabel(modal, text=LANG[self.lang]["settings_title"], font=("Helvetica", 18, "bold"), text_color=C_HIGHLIGHT).pack(pady=(20, 10))
        content = ctk.CTkFrame(modal, fg_color=C_PANEL, corner_radius=16)
        content.pack(fill="both", expand=True, padx=20, pady=10)

        motion_var = tk.BooleanVar(value=self.enable_animations)
        aurora_var = tk.BooleanVar(value=self.enable_aurora)
        contrast_var = tk.BooleanVar(value=self.high_contrast)

        ctk.CTkSwitch(content, text=LANG[self.lang]["settings_motion"], variable=motion_var).pack(anchor="w", padx=20, pady=12)
        ctk.CTkSwitch(content, text=LANG[self.lang]["settings_aurora"], variable=aurora_var).pack(anchor="w", padx=20, pady=12)
        ctk.CTkSwitch(content, text=LANG[self.lang]["settings_contrast"], variable=contrast_var).pack(anchor="w", padx=20, pady=12)

        def apply_settings():
            self.enable_animations = motion_var.get()
            self.enable_aurora = aurora_var.get()
            self.high_contrast = contrast_var.get()
            CONFIG.set("enable_animations", self.enable_animations)
            CONFIG.set("enable_aurora", self.enable_aurora)
            CONFIG.set("high_contrast", self.high_contrast)
            self.apply_visual_preferences()
            self.notify("Settings applied", C_ACCENT_2)
            self.log_activity("Settings updated")
            modal.destroy()

        ctk.CTkButton(modal, text=LANG[self.lang]["settings_save"], height=42, corner_radius=20, fg_color=C_HIGHLIGHT, text_color="#000", command=apply_settings).pack(pady=(0, 20))

    def update_faceit_status(self, text, color):
        if hasattr(self, "lbl_faceit_status"):
            self.lbl_faceit_status.configure(text=text, text_color=color)

    def set_faceit_loading(self, loading):
        if hasattr(self, "btn_faceit_link"):
            self.btn_faceit_link.configure(state="disabled" if loading else "normal")
        if hasattr(self, "btn_faceit_retry"):
            self.btn_faceit_retry.configure(state="disabled" if loading else "normal")
        if hasattr(self, "faceit_progress"):
            if loading:
                self.faceit_progress.start()
            else:
                self.faceit_progress.stop()
    def build_faceit_page(self, parent):
        ...
        self.btn_faceit_retry = ctk.CTkButton(inp, text=LANG[self.lang]["faceit_retry"], height=40, fg_color="#222", hover_color="#333", font=("Helvetica", 11, "bold"), command=lambda: self.run_thread(self.link_faceit_account))
        self.btn_faceit_retry.pack(side="left", padx=(10, 0))
        status_row = ctk.CTkFrame(parent, fg_color=C_PANEL, corner_radius=16)
        status_row.pack(fill="x", padx=10, pady=(0, 10))
        self.lbl_faceit_status_title = ctk.CTkLabel(status_row, text=LANG[self.lang]["faceit_status"], font=("Helvetica", 12, "bold"), text_color=C_DIM)
        self.lbl_faceit_status_title.pack(side="left", padx=16, pady=12)
        self.lbl_faceit_status = ctk.CTkLabel(status_row, text=LANG[self.lang]["faceit_ready"], font=("Helvetica", 12, "bold"), text_color=C_STATUS_GREEN)
        self.lbl_faceit_status.pack(side="left", padx=6)
        self.faceit_progress = ctk.CTkProgressBar(status_row, width=180, height=8, progress_color=C_FACEIT_ORANGE, fg_color=C_BORDER, mode="indeterminate")
        self.faceit_progress.pack(side="right", padx=16)
        self.faceit_progress.stop()
    ...
    def animate_loop(self):
        ...
        if self.enable_animations:
            try:
                if "..." in self.lbl_header.cget("text") or "Ready" in self.lbl_header.cget("text"):
                    self.lbl_header.configure(text_color=f"#{val:02x}{val:02x}{val:02x}")
            except: pass
            try:
    ...
    def toggle_lang(self):
        ...
        if hasattr(self, "btn_settings"):
            self.btn_settings.configure(text=d["action_settings"])
        if hasattr(self, "btn_diagnostics"):
            self.btn_diagnostics.configure(text=d["action_diagnostics"])
        if hasattr(self, "btn_faceit_retry"):
            self.btn_faceit_retry.configure(text=d["faceit_retry"])
        if hasattr(self, "lbl_faceit_status_title"):
            self.lbl_faceit_status_title.configure(text=d["faceit_status"])
        ...
        if hasattr(self, "lbl_health_title"):
            self.lbl_health_title.configure(text=d["health_title"])
        if hasattr(self, "health_items"):
            if "parser" in self.health_items:
                self.health_items["parser"]["label"].configure(text=d["health_parser"])
            if "pygame" in self.health_items:
                self.health_items["pygame"]["label"].configure(text=d["health_pygame"])
            if "openai" in self.health_items:
                self.health_items["openai"]["label"].configure(text=d["health_openai"])
            self.run_diagnostics()
        if hasattr(self, "lbl_faceit_status"):
            self.lbl_faceit_status.configure(text=d["faceit_ready"], text_color=C_STATUS_GREEN)
    ...
    def link_faceit_account(self):
        ...
        if not FACEIT_API_KEY:
            self.update_faceit_status(LANG[self.lang]["faceit_failed"], C_STATUS_RED)
            return self.notify("API Key Missing", C_ACCENT_1)
        self.set_faceit_loading(True)
        self.update_faceit_status(LANG[self.lang]["faceit_syncing"], C_STATUS_YELLOW)
        h = {"Authorization": f"Bearer {FACEIT_API_KEY}"}
        try:
            r = requests.get(f"https://open.faceit.com/data/v4/players?nickname={name}", headers=h, timeout=15)
            if r.status_code != 200:
                self.update_faceit_status(LANG[self.lang]["faceit_failed"], C_STATUS_RED)
                return self.notify("Player Not Found", C_ACCENT_1)
            pid = r.json()['player_id']
            self.notify("Fetching Matches...", C_ACCENT_3)
            matches = requests.get(f"https://open.faceit.com/data/v4/players/{pid}/history?game=cs2&offset=0&limit=20", headers=h, timeout=20).json().get('items', [])
            self.cached_matches = []
            for i, m in enumerate(matches):
                self.notify(f"Parsing {i+1}/{len(matches)}...", C_HIGHLIGHT); self.update_idletasks()
                mn, sc, res, kd, hs, durl = "Unknown", "- / -", "UNKNOWN", "-", "-", ""
                try:
                    rs = requests.get(f"https://open.faceit.com/data/v4/matches/{m['match_id']}/stats", headers=h, timeout=20)
                ...
                try:
                    match_details = requests.get(f"https://open.faceit.com/data/v4/matches/{m['match_id']}", headers=h, timeout=20).json()
            ...
            self.update_faceit_status(LANG[self.lang]["faceit_ready"], C_STATUS_GREEN)
        except Exception as e:
            self.update_faceit_status(LANG[self.lang]["faceit_failed"], C_STATUS_RED)
            self.notify(f"API Error: {e}", C_ACCENT_1)
        finally:
            self.set_faceit_loading(False)
